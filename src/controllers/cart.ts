...
export const addItemToCart: RequestHandler = async (req, res, next) => {
    const { restaurantId, productId, variantIndex, quantity, packagingWeight, type } = req.body;
    ...
    try {
        ...
        const priceDetail = variant.priceDetails.find(
            (pd) => pd.packagingWeight === packagingWeight && pd.type === type
        );
        ...
        let cart = await Cart.findOne({ restaurant: restaurantId });
...
export const updateCartItemQuantity: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        const variantIndex = product.variants.findIndex(variant =>
            variant.priceDetails.some(pd => pd.packagingWeight === cartItem.packagingWeight)
        );
        if (variantIndex === -1) return res.status(404).json({ message: 'Variant not found in product' });

        const variant = product.variants[variantIndex];
        const priceDetail = variant.priceDetails.find(pd => pd.packagingWeight === cartItem.packagingWeight);

        if (!priceDetail) return res.status(404).json({ message: 'Price detail not found' });
        if (quantity > priceDetail.inventory + cartItem.quantity) return res.status(400).json({ message: 'Insufficient stock' });
